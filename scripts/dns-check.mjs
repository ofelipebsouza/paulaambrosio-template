...
function providerFromMx(mxRecords) {
	...
	if (targets.includes('titan.email')) return TITAN;
	if (targets.includes('protection.outlook.com')) return MICROSOFT;
	return null;
}
...
report(
	spf ? (spf.includes(activeProvider.spfInclude.replace('include:', '')) ? 'ok' : 'warn') : 'fail',
	'SPF',
	spf ?? 'nenhum TXT v=spf1',
	spf
		? `O SPF existe mas não autoriza ${activeProvider.label}. Acrescente ${activeProvider.spfInclude} a: ${spf}`
		: `Adicione: TXT @ "v=spf1 ${activeProvider.spfInclude} ~all"`,
);
